...
describe('veTable cell style', () => {
  ...
  const wrapper = mount(veTable, {
    propsData: {
      ...
      cellStyleOption: {
        headerCellClass: ({ column }: { column: Record<string, unknown> }) => {
          if (column.field === 'hobby') {
            return 'table-header-cell-class'
          }
        },
        bodyCellClass: ({ column, rowIndex }: { column: Record<string, unknown>, rowIndex: number }) => {
          if (column.field === 'hobby') {
        ...
        footerCellClass: ({ column, rowIndex }: { column: Record<string, unknown>, rowIndex: number }) => {
