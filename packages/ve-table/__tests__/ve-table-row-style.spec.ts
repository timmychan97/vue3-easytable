...
describe('veTable row style', () => {
  ...
  it('rowStyleOption', () => {
    ...
    expect(wrapper.findAll('.ve-table-stripe').length).toBeGreaterThan(0)
    expect(wrapper.findAll('.ve-table-row-hover').length).toBeGreaterThan(0)
    expect(wrapper.findAll('.ve-table-row-highlight').length).toBeGreaterThan(0)
  })
  ...
  it('row highlight by setHighlightRow method', async () => {
    ...
    ;(wrapper.vm as any).setHighlightRow({ rowKey: 2 })
