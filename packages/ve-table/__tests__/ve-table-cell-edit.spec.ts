...
import CellEditor from '@/ve-table/src/editor/index'
...
describe('veTable cell edit', () => {
  ...
  it('cell editor render', async () => {
    const wrapper = mount(veTable, {
      propsData: {
        ...
        editOption: {
          // cell value change
          afterCellValueChange: ({ _row, _column }) => {},
        },
  ...
  it('beforeStartCellEditing callback', async () => {
    ...
    ;(wrapper.vm as any).startEditingCell({
      rowKey: 0,
      colKey: 'name',
      defaultValue: 'AAA',
    })
    ...
    await later()
    ;(wrapper.vm as any).stopEditingCell()
    await later()
