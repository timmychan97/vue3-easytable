:::anchor Kontrollerbar egenskap

:::demo 1. `selectedRowKeys` er den "kontrollerbare egenskapen" for flervalg. Etter valg ma du tilordne verdier pa nytt i `selectedRowChange`- og `selectedAllChange`-hendelsene. Denne egenskapen lar deg tilpasse ytterligere funksjonalitet<br>2. Etter at `selectedRowKeys`-egenskapen er satt, vil `defaultSelectedRowKeys` og `defaultSelectedAllRows`-egenskapene bli ignorert
